    ...
    def plot_soft_edge_mask(
        ...
        **kwargs,
        ...
        if self.dataset_name.lower() in ["ba_2motifs", "ba_lrp"]:
            nodelist, edgelist = self.get_topk_edges_subgraph(
                edge_index, edge_mask, top_k, un_directed
            )
            self.plot_ba2motifs(
                graph,
                nodelist,
...
class PGExplainer(nn.Module):
    ...
    def forward(
        ...
        if self.explain_graph:
            ...
            masked_node_list = [
                node for node in range(data.x.shape[0]) if node in selected_nodes
            ]
            maskout_nodes_list = [
                node for node in range(data.x.shape[0]) if node not in selected_nodes
            ]
            value_func = GnnNetsGC2valueFunc(self.model, target_class=label)

            masked_pred = gnn_score(
                masked_node_list,
                data,
            ...
            value_func = GnnNetsNC2valueFunc(
                self.model, node_idx=new_node_idx, target_class=label
            )
            ...
            sparsity_score = sparsity(
                masked_node_list, data, subgraph_building_method="zero_filling"
            )
    ...
    def visualization(
        ...
        if self.explain_graph:
            ...
            ), "visualization method doesn't get the target node index"
            x, edge_index, y, subset, kwargs = self.get_subgraph(
                node_idx=node_idx, x=data.x, edge_index=data.edge_index, y=data.y
            )
            new_node_idx = torch.where(subset == node_idx)[0]

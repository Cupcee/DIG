...
from dig.xgraph.method import CFPGExplainer
...
@hydra.main(config_path="benchmarks/xgraph/config", config_name="config")
def pipeline(config):
    config.models.gnn_saving_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cf_pgexplainer_checkpoints')
    ...
    pgexplainer = CFPGExplainer(model,
    ...
    if os.path.isfile(pgexplainer_saving_path):
        print("Load saved CFPGExplainer model...")
...
if __name__ == '__main__':
    import sys
    sys.argv.append('explainers=cf_pgexplainer')
    pipeline()
